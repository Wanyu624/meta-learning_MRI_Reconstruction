...
####################################################################################################
    def block(self, layers1, alpha1, tau1):
...
#            x1 = u1
    ...
    def forward(self, n):
        ...
        for i in range(n):     
            [x1, alpha1, tau1] = self.block(layers1[-1], a1[-1], t1[-1])
...
layers = model.forward(PhaseNumber)
...
learning_rate = tf.train.exponential_decay(learning_rate = 0.0004,
...
lamda = tf.train.exponential_decay(learning_rate = 1e-5,
                                       global_step = global_step,
                                       decay_steps = 10000,
                                       decay_rate=1.1, staircase=False)
...
#init_Weights = 'Ratio_vali_8' + '/Saved_Model_4.ckpt' 
...
#            output_data = "Epoch [%02d/%02d], lamda: %.7f, stop_ct: %.7f, gradient: %.7f, train loss: %.7f, valid loss: %.7f, lr: %.7f \n" % ( epoch_p, EpochNum, 
#                                 self.session.run(lamda), self.session.run(stop_ct, feed_dict=feed_dict), self.session.run(loss_trnorm, feed_dict=feed_dict),
#                                 self.session.run(loss_tr, feed_dict=feed_dict), self.session.run(loss_vl, feed_dict=feed_dict), 
#                                 self.session.run(learning_rate))
#            print(output_data)
...
for imag_no in range(ntest):
    ...
    target_te = np.tile(target_te, (8, 1, 1))
    ...
    kspace_te = np.vstack((kspace10, kspace20, kspace30, kspace40, kspace10, kspace20, kspace30, kspace40))#(5, 160, 180)
    ...
    rec = np.reshape(Prediction_value, (8,160,180))
    rec = rec.astype(np.float32)
    reference = np.reshape(u, (8,160,180))
    reference = reference.astype(np.float32)
    
#    PSNR1 =  psnr(np.squeeze(reference[:1]), np.squeeze(rec[:1]))
#    SSIM1 =  ssim(np.squeeze(reference[:1]), np.squeeze(rec[:1]))
#    NMSE1 =  nmse(np.squeeze(reference[:1]), np.squeeze(rec[:1]))
#    PSNR2 =  psnr(np.squeeze(reference[1:2]), np.squeeze(rec[1:2]))
#    SSIM2 =  ssim(np.squeeze(reference[1:2]), np.squeeze(rec[1:2]))
#    NMSE2 =  nmse(np.squeeze(reference[1:2]), np.squeeze(rec[1:2]))
#    PSNR3 =  psnr(np.squeeze(reference[2:3]), np.squeeze(rec[2:3]))
#    SSIM3 =  ssim(np.squeeze(reference[2:3]), np.squeeze(rec[2:3]))
#    NMSE3 =  nmse(np.squeeze(reference[2:3]), np.squeeze(rec[2:3]))
#    PSNR4 =  psnr(np.squeeze(reference[3:4]), np.squeeze(rec[3:4]))
#    SSIM4 =  ssim(np.squeeze(reference[3:4]), np.squeeze(rec[3:4]))
#    NMSE4 =  nmse(np.squeeze(reference[3:4]), np.squeeze(rec[3:4]))
    PSNR1 =  psnr(np.squeeze(reference[4:5]), np.squeeze(rec[4:5]))
    SSIM1 =  ssim(np.squeeze(reference[4:5]), np.squeeze(rec[4:5]))
    NMSE1 =  nmse(np.squeeze(reference[4:5]), np.squeeze(rec[4:5]))
    PSNR2 =  psnr(np.squeeze(reference[5:6]), np.squeeze(rec[5:6]))
    SSIM2 =  ssim(np.squeeze(reference[5:6]), np.squeeze(rec[5:6]))
    NMSE2 =  nmse(np.squeeze(reference[5:6]), np.squeeze(rec[5:6]))
    PSNR3 =  psnr(np.squeeze(reference[6:7]), np.squeeze(rec[6:7]))
    SSIM3 =  ssim(np.squeeze(reference[6:7]), np.squeeze(rec[6:7]))
    NMSE3 =  nmse(np.squeeze(reference[6:7]), np.squeeze(rec[6:7]))
    PSNR4 =  psnr(np.squeeze(reference[7:8]), np.squeeze(rec[7:8]))
    SSIM4 =  ssim(np.squeeze(reference[7:8]), np.squeeze(rec[7:8]))
    NMSE4 =  nmse(np.squeeze(reference[7:8]), np.squeeze(rec[7:8]))
    ...
    PSNR3_All.append(PSNR3)
    SSIM3_All.append(SSIM3)
    NMSE3_All.append(NMSE3)

    PSNR4_All.append(PSNR4)
    SSIM4_All.append(SSIM4)
    NMSE4_All.append(NMSE4)
    
    TIME_ALL.append((end - start))
 
output_data1 = "ratio10 ckpt NO. is %d, Avg REC PSNR is %.4f dB std %.4f, SSIM is %.4f std %.4f, NMSE is %.4f std %.4f, time: %.4f\n" % (ckpt_model_number, np.mean(PSNR1_All), np.std(PSNR1_All), np.mean(SSIM1_All), np.std(SSIM1_All), np.mean(NMSE1_All), np.std(NMSE1_All), np.mean(TIME_ALL))
print(output_data1)
output_data2 = "ratio20 ckpt NO. is %d, Avg REC PSNR is %.4f dB std %.4f, SSIM is %.4f std %.4f, NMSE is %.4f std %.4f, time: %.4f\n" % (ckpt_model_number, np.mean(PSNR2_All), np.std(PSNR2_All), np.mean(SSIM2_All), np.std(SSIM2_All), np.mean(NMSE2_All), np.std(NMSE2_All), np.mean(TIME_ALL))
print(output_data2)
output_data3 = "ratio30 ckpt NO. is %d, Avg REC PSNR is %.4f dB std %.4f, SSIM is %.4f std %.4f, NMSE is %.4f std %.4f, time: %.4f\n" % (ckpt_model_number, np.mean(PSNR3_All), np.std(PSNR3_All), np.mean(SSIM3_All), np.std(SSIM3_All), np.mean(NMSE3_All), np.std(NMSE3_All), np.mean(TIME_ALL))
print(output_data3)
output_data4 = "ratio40 ckpt NO. is %d, Avg REC PSNR is %.4f dB std %.4f, SSIM is %.4f std %.4f, NMSE is %.4f std %.4f, time: %.4f\n" % (ckpt_model_number, np.mean(PSNR4_All), np.std(PSNR4_All), np.mean(SSIM4_All), np.std(SSIM4_All), np.mean(NMSE4_All), np.std(NMSE4_All), np.mean(TIME_ALL))
print(output_data4)
...
output_file.write(output_data4)
output_file.close()
sess.close()
